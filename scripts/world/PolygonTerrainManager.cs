...
public partial class PolygonTerrainManager : Node
{
    ...
    /// <summary>Half-extent of the outer walkable nav boundary in pixels.</summary>
    ...
    private static List<Vector2[]> TraceContours(Dictionary<Vector2I, List<Vector2I>> graph)
    {
    ...
    {
        var navPoly    = new NavigationPolygon { AgentRadius = 8f };
        ...
        foreach (var child in _blobContainer.GetChildren())
        {
            ...
            foreach (var bodyChild in body.GetChildren())
                if (bodyChild is CollisionPolygon2D cp)
                    sourceData.AddObstructionOutline(cp.Polygon);
        }
